...
{
    public class CostTestFail : Cost
    {
        public CostTestFail(int food, int wood, int gold, int stone) : base(food, wood, gold, stone)
        {
            Food = 100;
            Wood = 1000;
            Gold = 10000;
    ...
    public void Setup()
    {
        _cost = new CostTestFail(100, 1000, 10000, 100000);
    }
    ...
    {
        Assert.That(_cost.Gold, Is.EqualTo(10));
    }
    
    [Test]
    public void TestDeLaVerificacionDelCostoDePiedra()
    {
        Assert.That(_cost.Stone, Is.EqualTo(100));
    }
}